...
// Split out of the entry chunk: ProjectView pulls in CodeMirror, xterm and the
// debugger, none of which a signed-out visitor on the landing page needs.
...
function App() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-3 z-40 mx-auto mb-6 mt-3 flex w-[calc(100%-1.5rem)] max-w-[1600px] items-center justify-between gap-3 rounded-pill border border-border bg-card/80 px-5 py-2.5 shadow-elegant backdrop-blur-md">
        <button
          type="button"
          className="flex items-center gap-2 text-lg font-bold tracking-tight"
          onClick={goToDashboard}
        >
          <span aria-hidden="true" className="bg-gradient-primary bg-clip-text text-transparent">
            ◆
          </span>
          CodeMesh
        </button>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" className="rounded-md border border-border bg-card/70 px-3 py-1.5 text-sm transition-colors hover:border-primary/50 hover:text-primary" onClick={goToGallery}>
            Explore
          </button>
      ...
      <main className="mx-auto w-full max-w-[1600px] flex-1 px-3">
        <Suspense
          fallback={<div className="px-2 py-10 text-center text-sm text-muted-foreground">Loading…</div>}
        >
      ...
      {showFooter && (
        <footer className="mt-10 flex items-center justify-center gap-5 border-t border-border py-6 text-sm text-muted-foreground">
          <button type="button" className="transition-colors hover:text-primary" onClick={() => goToLegal('privacy')}>
            Privacy
          </button>
          <button type="button" className="transition-colors hover:text-primary" onClick={() => goToLegal('terms')}>
            Terms
          </button>
        </footer>
      )}
